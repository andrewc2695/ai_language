...
export const updateWordProgress = tool({
  ...
  inputSchema: z.object({
    words: z.array(
      z.object({
        ...
        success: z.boolean().describe("Whether the user translated this word correctly"),
      })
  ...
  execute: async ({ words }) => {
    ...
    const getWord = db.prepare(`SELECT id, english, jyutping, proficiency_level FROM words WHERE id = ?`);
    ...
    const updateAll = db.transaction((words: { id: number; success: boolean }[]) => {
      for (const word of words) {
        const before = getWord.get(word.id) as { id: number; english: string; jyutping: string; proficiency_level: number } | undefined;
        if (word.success) {
          updateSuccess.run(word.id);
        } else {
          updateFail.run(word.id);
        }
        const after = getWord.get(word.id) as { proficiency_level: number } | undefined;
        console.log(
